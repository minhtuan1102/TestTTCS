...
using Photon.Pun; // Đã thêm từ trước

public class Player : MonoBehaviourPun // Đã kế thừa MonoBehaviourPun từ trước
{
    ...
    void Update()
    {
        if (!photonView.IsMine) return; // Chỉ xử lý cho người chơi hiện tại
        ...
        if (Input.GetKeyDown(KeyCode.Q) && !isDashing)
        {
            ...
            isDashing = true;
            dashTime = 0;
            photonView.RPC("StartDash", RpcTarget.All);
        }
    ...
    // Thêm lại các phương thức TriggerRecoil và TriggerSwing
    ...
    void FixedUpdate()
    {
        if (!photonView.IsMine && !isDashing) return;
        ...
        if (photonView.IsMine)
        {
            ...
            photonView.RPC("SyncHand", RpcTarget.All, skin.flipX, localScale.y);
    ...
    [PunRPC]
    void SyncHand(bool flipX, float scaleY)
    {
        skin.flipX = flipX;
        inverse = flipX ? -1f : 1f;
        Vector3 localScale = main_hand.transform.localScale;
